...
const Record = () => {
  const headData = { title: '交易记录', back: 'goBack' }
    ...
    <div className='assets_record'>
      ...
      <ul className="public_filter">
        <li onClick={ () => filter('currency') } className='public_filter_i'>
          ...
          {
            filterData.currency ? <i onClick={ (event) => close('currency', event) } className='iconfont icon-guanbi' /> : <i className='iconfont icon-xiangyou1' />
          }
        </li>
        <li onClick={ () => filter('date') } className='public_filter_i'>
          <p>{ filterData.date ? filterData.date : '时间' }</p>
          {
            filterData.date ? <i onClick={ (event) => close('date', event) } className='iconfont icon-guanbi' /> : <i className='iconfont icon-xiangyou1' />
          }
        </li>
      </ul>
      <Picker
