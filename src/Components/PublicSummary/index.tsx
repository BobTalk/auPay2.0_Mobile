...
const PublicSummary = (props: propsVali) => {
  return (
    <div
      ...
      className={mergeClassName(styleScope["summary-box"], "grid")}
    >
      <i
      ...
      <div>
        ...
        {
          props.summary.map((item: any, index: number) => <li style={props.summaryStyle} className={styleScope['summary-item']} key={item}>{index+1}.{item}</li> )
        }
      </div>
    </div>
  );
